async function searchSubmit(form,db){
  let type_options = ["journal","travel notes","AW's journal","journal index","travel accounts","itinerary"];
  ...
  let result_wyas = [];  let result_alcb =[];
  let date_list_wyas = []; let date_list_alcb = [];

  if(query.search_mode==='like'){
      let reg = wc2RegExp(query.keyword);
  }
  ...
  document.getElementById('wrapper').innerHTML = '<span style="font-size:15px; margin-top:5px;">Searching...</span>';
  // query in both tables
  if(query.search_mode==='match'){
    try{
      result_wyas = await db.exec(`select highlight(wyas_fts, 0, '<span class="highlight">', '</span>'),link,date,type,part from wyas_fts where text match ? AND text NOTNULL`,[query.keyword]);
      result_alcb = await db.exec(`select highlight(alcb_fts, 0, '<span class="highlight">', '</span>'),credit,link,date,type,part from alcb_fts where text match ? AND text NOTNULL`,[query.keyword]);   
    }catch(err){
    ...
    try{
      result_wyas = await db.exec(`select highlight(wyas_exact_fts, 0, '<span class="highlight">', '</span>'),link,date,type,part from wyas_exact_fts where text match ? AND text NOTNULL`,[query.keyword]);
      result_alcb = await db.exec(`select highlight(alcb_exact_fts, 0, '<span class="highlight">', '</span>'),credit,link,date,type,part from alcb_exact_fts where text match ? AND text NOTNULL`,[query.keyword]);   
    }catch(err){
      document.getElementById('wrapper').innerHTML = '<span style="font-size:15px; margin-top:5px;">' + err + '</span>';
      return;
    }   
    ...
    try{
      result_wyas = await db.exec(`select text,link,date,type,part from wyas_fts where text like ? AND text NOTNULL`,[query.keyword]);
      result_alcb = await db.exec(`select text,credit,link,date,type,part from alcb_fts where text like ? AND text NOTNULL`,[query.keyword]);   
    }catch(err){
      document.getElementById('wrapper').innerHTML = '<span style="font-size:15px; margin-top:5px;">' + err + '</span>';
      return;
    }      
...
function yearHitsHeat(merged_result){
    ...
    if (Math.max.apply(null,year_hits)===0){
      ...
      // var colorGradient = d3.interpolate('#ffffff','#000399');
...
//     loadingBar = document.querySelector(".progress-bar")
//     var res = new Object;
...
//     // const dataPromise = fetch("../data/journal.db").then(async (res) =>{
//     //     const reader = res.body.getReader();
//     //     const total = res.headers.get('content-length');
//     //     let loaded = 0;
//     //     let chunks = new Uint8Array(total);
//     //     while(true) {
...
//     //       loadingBar.innerText = `${(loaded / total * 100).toFixed(0)}%`
...
//     // let total = null;
//     // // let chunks = new Uint8Array() // array of received binary chunks (comprises the body)
...
//     //     return reader.read().then(push);
//     // };
//     // dataPromise = fetch("https://raw.githubusercontent.com/jiangjy-713/AL_Index/master/data/journal.db")
...
// var db = loadDB();
var fts_result = [];
document.getElementById('search').addEventListener('submit',async function(){
  if (typeof(db)!=="undefined"){
    fts_result =  searchSubmit(this,db);
  }else{
    fts_result =  await searchSubmit(this,worker.db);
  }
